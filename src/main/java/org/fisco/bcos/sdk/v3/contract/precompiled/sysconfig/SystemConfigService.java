...
public class SystemConfigService {
	...
	static {
		...
		predicateMap.put(TX_GAS_LIMIT, value -> value.compareTo(BigInteger.valueOf(TX_GAS_LIMIT_MIN)) >= 0);
	}

	public SystemConfigService(Client client, CryptoKeyPair credential) {
		this.client = client;
		this.systemConfigPrecompiled = SystemConfigPrecompiled
				.load(client.isWASM() ? PrecompiledAddress.SYS_CONFIG_PRECOMPILED_NAME
						: PrecompiledAddress.SYS_CONFIG_PRECOMPILED_ADDRESS, client, credential);
	}
	...
	public RetCode setValueByKey(String key, String value) throws ContractException {
		if (COMPATIBILITY_VERSION.equals(key) && !checkCompatibilityVersion(client, value)) {
			String nodeVersionString = client.getGroupInfo().getResult().getNodeList().stream()
					.map(node -> node.getIniConfig().getBinaryInfo().getVersion()).collect(Collectors.joining(","));
			throw new ContractException("The compatibility version " + value
		...
		return ReceiptParser.parseTransactionReceipt(receipt,
				tr -> systemConfigPrecompiled.getSetValueByKeyOutput(receipt).getValue1());
	}
	...
	static boolean checkAvailableFeatureKeys(Client client, String key) {
		...
		Optional<GroupInfo> group = client.getGroupInfoList().getResult().stream().filter(groupInfo -> {
			return groupInfo.getGroupID().equals(client.getGroup());
		}).findFirst();
		if (!group.isPresent()) {
			logger.warn("Not found group! {} {}", client.getGroup(), client.getGroupInfoList().getResult());
			return true;
		}
		...
		for (BcosGroupNodeInfo.GroupNodeInfo groupNodeInfo : group.get().getNodeList()) {
			...
			if (featureKeys == null) {
				if (groupNodeInfo.getProtocol().getCompatibilityVersion() == EnumNodeVersion.BCOS_3_2_3.toVersionObj()
	...
	public static boolean checkCompatibilityVersion(Client client, String version) {
		try {
			...
			List<BcosGroupNodeInfo.GroupNodeInfo> nodeList = client.getGroupInfo().getResult().getNodeList();
			return nodeList.stream().allMatch(node -> setVersion
					.compareTo(EnumNodeVersion.getClassVersion(node.getIniConfig().getBinaryInfo().getVersion())) <= 0);
		} catch (Exception e) {
			return false;
		}
	}
