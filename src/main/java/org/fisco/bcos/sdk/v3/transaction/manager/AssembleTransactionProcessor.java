...
import org.fisco.bcos.sdk.v3.test.transaction.model.dto.ResultCodeEnum;
...
public class AssembleTransactionProcessor extends TransactionProcessor
    ...
    public String deployOnly(
        ...
        if (log.isDebugEnabled()) {
            log.debug(
                    "deploy, abiSize:{}, binSize:{}, params:{}, path:{}",
                    abi.length(),
                    bin.length(),
                    params,
                    path);
    ...
    public String deployAsync(
        ...
        if (log.isDebugEnabled()) {
            log.debug(
                    "deployAsync, abiSize:{}, binSize:{}, params:{}, path:{}",
                    abi.length(),
                    bin.length(),
                    params,
                    path);
    ...
    public TransactionResponse sendTransactionAndGetResponse(
            ...
            throws ContractCodecException {
        if (log.isDebugEnabled()) {
            log.debug("sendTx, to:{}, func:{}, params:{}", to, functionName, params);
        }
    ...
    public void sendTransactionAsync(
            ...
            throws ContractCodecException {
        if (log.isDebugEnabled()) {
            log.debug("sendTxAsync, to:{}, func:{}, params:{}", to, functionName, params);
        }
        byte[] data = this.encodeFunction(abi, functionName, params);
        int txAttribute = 0;
        if (client.isWASM()) {
    ...
    public void sendTransactionAndGetReceiptByContractLoaderAsync(
        ...
        if (log.isDebugEnabled()) {
            log.debug(
                    "sendTxAsync, name:{}, to:{}, func:{}, params:{}",
                    contractName,
                    contractAddress,
                    functionName,
                    args);
    ...
    public CallResponse sendCall(
            ...
            throws TransactionBaseException, ContractCodecException {
        if (log.isDebugEnabled()) {
            log.debug("call, to:{}, func:{}, params:{}", to, functionName, paramsList);
        }
    ...
    @Override
    public CallResponse sendCallWithSign(
            String from, String to, String abi, String functionName, List<Object> params)
            throws TransactionBaseException, ContractCodecException {
        if (log.isDebugEnabled()) {
            log.debug("callWithSign, to:{}, func:{}, params:{}", to, functionName, params);
        }
        ...
        for (ABIDefinition definition : abiDefinitions) {
            try {
                abiDefinition = definition;
                data = this.contractCodec.encodeMethodByAbiDefinition(definition, params);
            } catch (ContractCodecException e) {
                // single one method, throw e directly
                if (abiDefinitions.size() == 1) {
                    throw e;
        ...
        return this.callWithSignAndGetResponse(from, to, abiDefinition, data);
    }
    ...
    public void sendCallAsync(
            ...
            throws ContractCodecException {
        if (log.isDebugEnabled()) {
            log.debug("callAsync, to:{}, func:{}, params:{}", to, functionName, params);
        }
        List<ABIDefinition> abiDefinitions = getAbiDefinition(abi, functionName, params.size());
        if (abiDefinitions == null || abiDefinitions.isEmpty()) {
            throw new ContractCodecException(Constant.NO_APPROPRIATE_ABI_METHOD);
    ...
    @Override
    public CallResponse sendCallWithSignWithStringParams(
            String from, String to, String abi, String functionName, List<String> params)
            throws TransactionBaseException, ContractCodecException {
        List<ABIDefinition> abiDefinitions = getAbiDefinition(abi, functionName, params.size());
        if (log.isTraceEnabled()) {
            log.trace(
                    "sendCallWithSignWithStringParams, to:{}, functionName:{}, params:{}",
                    to,
                    functionName,
                    params);
        }
    ...
    public void callAndGetResponseAsync(
            String from,
            String to,
